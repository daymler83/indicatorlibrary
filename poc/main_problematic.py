...
def create_indicator(indicator_data: IndicatorCreate, db: Session = Depends(get_db)):
    ...
    history = IndicatorHistory(
        ...
        changed_by="system_create"  # or current_user if you have auth
    )
...
def get_indicator(indicator_id: str, db: Session = Depends(get_db)):
    ...
    history = IndicatorHistory(
        ...
        changed_by="system_update"  # or current_user if available
    )
    db.add(history)
...
async def upload_csv(
    ...
    if file_type == "indicators":
        ...
        for _, row in df.iterrows():
            ...
            # Normalize keys that may be problematic
            ...
            year = int(row_data.get("year")) if row_data.get("year") else None
            ...
            if existing:
                # 🔁 Log previous state to value_history
                db.execute(
                    """
