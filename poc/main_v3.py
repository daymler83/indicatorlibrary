...
def dashboard(
    ...
    indicator_query = db.query(Indicator)
    
    if sector:
        indicator_query = indicator_query.filter(Indicator.sector == sector)
    if status:
        indicator_query = indicator_query.filter(Indicator.status == status)
    if dimension:
        indicator_query = indicator_query.filter(Indicator.dimension == dimension)
    if search:
        indicator_query = indicator_query.filter(Indicator.name.ilike(f'%{search}%'))
    ...
    sectors = db.query(Indicator.sector).distinct().filter(Indicator.sector.isnot(None)).all()
    sectors = [s[0] for s in sectors]
    
    statuses = db.query(Indicator.status).distinct().all()
    statuses = [s[0] for s in statuses]
    
    dimensions = db.query(Indicator.dimension).distinct().filter(Indicator.dimension.isnot(None)).all()
    dimensions = [d[0] for d in dimensions]
    
    # Group by dimension for accordion
    dimension_groups = {}
