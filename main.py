...
def tracking_dashboard(
    ...
    # Base query with tracking status filter
    query = db.query(IndicatorValue).filter(IndicatorValue.tracking_status.isnot(None))

    # Apply filters if they exist
    if year:
        query = query.filter(IndicatorValue.year == year)
    if region:
        query = query.filter(IndicatorValue.region == region)
    if province:
        query = query.filter(IndicatorValue.province == province)
    if gender:
        query = query.filter(IndicatorValue.gender == gender)

    # Get filtered values
    values = query.order_by(IndicatorValue.year.desc()).all()
    ...
    filter_results = filter_query.all()
    
    # Extract unique values for each filter
    filter_years = sorted({r.year for r in filter_results if r.year}, reverse=True)
    filter_regions = sorted({r.region for r in filter_results if r.region})
    filter_provinces = sorted({r.province for r in filter_results if r.province})
    filter_genders = sorted({r.gender for r in filter_results if r.gender})
